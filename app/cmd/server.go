...
func (s ServerCmd) Run(ctx context.Context) error {
	...
	if _, ok := ex["db"].(string); !ok {
		return errors.New("can't find db in mongo url")
	}
	if _, ok := ex["collection"].(string); !ok {
		return errors.New("can't find collection in mongo url")
	}
